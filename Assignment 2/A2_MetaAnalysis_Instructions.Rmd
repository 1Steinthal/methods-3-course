...
pacman::p_load(tidyverse, brms, pracma)
```
...
#Predicting the outcome of studies
effect_size_means<-rnorm(n_studies, .4, .4)
d<-tibble(ID=seq(1:n_studies),participants, effect_size_means)

d<-d %>% 
  mutate(SE=std_err(rnorm(participants,effect_size_means,0.8)))

for(i in 1:n_studies){
  d$SE[i]<-std_err(rnorm(participants[i],effect_size_means[i],0.8))
}

#standard_error<-sd(effect_sizes)/sqrt(n_studies)
std_err(d$effect_size_means)
