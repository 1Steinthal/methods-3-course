...
#Prepping the R-program with the necessary packets

knitr::opts_chunk$set(echo = TRUE)

#install.packages("tidyverse", "tidymodels", "brms", "cmdstanr", "readr", "caret")
#library(tidyverse, tidymodels, brms, cmdstanr, readr, caret)
...
for(i in seq(10)){
  ...
  if (i == 1) {
    ...
    base_df_true <- base_df #Little...
  }
  else{
    meta_df_true <- rbind(meta_df_true, meta_df)
    base_df_true <- rbind(base_df_true,base_df) #... by little
  }
...
for (i in seq(nrow(meta_df))){ #Simulate data
...
meta_df <- meta_df %>%  #The names seemed to be off which is why we made this little trick
  rename('pitch_var' = v1,
         'speech_rat' = v2,
         'pause_dur' = v3,
         'pause_num' = v4,
...
plot_meta_df <- melt(meta_df) #Assessing quality
...
rec_base <- train_base %>%  #Also for noice-set
  recipe(Group ~ . ) %>% 
  step_scale(all_numeric() ) %>% 
  step_center(all_numeric() ) %>% 
...
test_base_s <- bake(rec_base, new_data = test_base) #Bakeries running
...
#Using all machines on both datasets
...
#Cutting out the scores we need
...
#Briefly testing all 4 machines
...
emp_plot <- tibble(
  ...
  mutate(F1 = CM_sim) #And all the fuzzle to get the F1-score
```
...
###### FIRST SAMPLING - TAKING OUT THE FINAL TESTSET
set.seed(123)
pca_rec_j <- d %>% 
...
###### We save these datasets for later

d<-train_emp_pca
final_train <- d
final_test <- test_emp_pca
...
#Fucntion for simulating with x (nums) components
PCA<-function(nums) {
...
ggplot(emp_plot, aes(id,F1,colour = kappa))+
  ...
  xlab("Number of PC")+
  ylab("F1-score")+
  ggtitle("F1-scores dependent on the number of PC's using the svm-engine - NO TESTSET USED YET")




### AND NOW WE HAVE THE FINAL TEST-SET LYING. We found that 20 PC was the most appropriate
...
PCA<-function(nums) {
pca_rec <- recipe(~., data = final_train) %>%
  update_role(Diagnosis, new_role = "id") %>%
  step_normalize(all_predictors()) %>%
  step_pca(all_numeric_predictors(), num_comp = nums)

pca_prep <- prep(pca_rec, training = final_train)
pca_rec_j <- juice(pca_prep)

test_emp_s <- bake(pca_prep, new_data = final_test)
...
emp_plot<-rbind(PCA(10),PCA(20), PCA(30)) #The 10 and 30 are mainly for comparison. We go with 20
...
ggplot(emp_plot, aes(id,F1,colour = kappa))+
  ...
  xlab("Number of PC")+
  ylab("F1-score")+
  ggtitle("F1-scores dependent on the number of PC's using the svm-engine - WITH TESTSET. We aimed for the 20 components based on our training")
